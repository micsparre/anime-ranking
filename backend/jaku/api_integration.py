...
def get_anime_obj(id):
    ...
    """
    raw_data = fetch_api(id, query)[0]
    anime_obj = build_anime_obj(raw_data)
    return anime_obj


def fetch_titles(title):
    """
    Returns a list of Anime objects from the third-party API, given a search query.
    """
    query = """
        query ($query: String) {
            AnimeSearch: Page {
    ...
    """
    return fetch_api(title, query)


def build_anime_obj(data):
    """
    ...
    if (
        data["startDate"]["year"] is None
        or data["startDate"]["month"] is None
        or data["startDate"]["day"] is None
    ):
        start_date = None
    else:
        start_date = date(
    ...
    if (
        data["endDate"]["year"] is None
        or data["endDate"]["month"] is None
        or data["endDate"]["day"] is None
    ):
        end_date = None
    else:
        end_date = date(
